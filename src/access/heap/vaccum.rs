...
impl Vacuum {
    pub fn new(bpm: Arc<BufferPoolManager>, tm: Arc<TransactionManager>, sm: Arc<StorageManager>) -> Self {
        Self { bpm, tm, sm }
    }
    ...
    pub fn vacuum_table(&self, table_oid: u32, table: Arc<RwLock<Table>>) -> Result<(), AccessError> {
        let num_pages = table.read()
            .map_err(|_| AccessError::Lock(LockError))?.num_pages()?;
    ...
    fn first_pass_page(&self, table_oid: u32, page_idx: u32, horizon: &Snapshot, table: Arc<RwLock<Table>>) 
        ...
        let mut page_lock = frame.data.write().map_err(|_| AccessError::Lock(LockError))?;
        ...
        for slot in 1..=num_slots {
            let action = if let Some(raw_bytes) = page_lock.get_item(slot) {
                ...
                let action = self.decide_vacuum_action(&view.header, view.header.read_infomask(), horizon)?;
                action
            } else {
                VacuumAction::None
            };
    ...
    fn second_pass_page(&self, table_oid: u32, page_idx: u32, table: Arc<RwLock<Table>>) -> Result<(), AccessError> {
        let tag = BufferTag { table_oid, page_idx };
        let mut t_lock = table.write().unwrap();
        let frame = self.bpm.fetch_page(tag, &mut t_lock)?;
    ...
    fn decide_vacuum_action(&self, header: &HeapTupleHeaderData, mask: TupleInfoMask, horizon: &Snapshot) -> Result<VacuumAction, AccessError> {
        let mut to_add = TupleInfoMask::empty();
        let clog = self.tm.clog.read().map_err(|_| AccessError::Lock(LockError))?;
        ...
        if is_dead {
            if !to_add.is_empty() {
                Ok(VacuumAction::UpdateHints { to_add, to_remove: TupleInfoMask::empty(), mark_dead: true })
            } else {
                Ok(VacuumAction::MarkDead)
            }
        } else if !to_add.is_empty() {
            Ok(VacuumAction::UpdateHints { to_add, to_remove: TupleInfoMask::empty(), mark_dead: false })
        } else {
            Ok(VacuumAction::None)
        }
    }
...
impl Vacuum {
    ...
    pub fn vacuum_all_tables(&self) -> Result<(), AccessError> {
        let storage = self.sm.as_ref();
        let class_table = storage.get_table(RG_CLASS_OID)?;
        HeapScan::new(self.bpm.clone(), class_table, self.tm.clone())
            .map_err(|_| AccessError::Lock(LockError))?
            .map(|tuple| RGClass::from_tuple(&tuple).oid as u32)
            .for_each(|oid| {
                if let Ok(table) = storage.get_table(oid) {
                    let _ = self.vacuum_table(oid, table);
                }
            });
        Ok(())
    }
